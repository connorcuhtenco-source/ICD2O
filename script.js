...
// --- Our Games hover preview: animated canvas mini-demos above the cursor ---
const gamePreviewFloat = document.getElementById('gamePreviewFloat');
const gamePreviewCanvas = document.getElementById('gamePreviewCanvas');
const gamePreviewLabel = document.getElementById('gamePreviewLabel');
const gamePreviewCtx = gamePreviewCanvas?.getContext('2d');

let gamePreviewActive = null;
let gamePreviewFrameId = null;
let gamePreviewStart = 0;

/** Map each tile class to a preview theme key and drawer function. */
const GAME_PREVIEW_DRAWERS = {
    tag: drawTagZonePreview,
...
function getGamePreviewKey(tile) {
    if (tile.classList.contains('game-tile-tag')) return 'tag';
    if (tile.classList.contains('game-tile-kill')) return 'kill';
...
/** Top-down Tag Zone: grass, player dodging taggers, orbiting power-up. */
function drawTagZonePreview(ctx, w, h, t) {
    ctx.fillStyle = '#2d8a45';
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = '#3aa856';
    for (let i = 0; i < 8; i++) {
        const gx = ((i * 37 + Math.floor(t * 12)) % (w + 20)) - 10;
        const gy = ((i * 53) % h);
        ctx.fillRect(gx, gy, 14, 10);
    }

    const px = w * 0.5 + Math.sin(t * 2.4) * (w * 0.28);
    const py = h * 0.55 + Math.cos(t * 1.8) * (h * 0.18);
    const taggers = [
        { x: w * 0.22 + Math.sin(t * 1.2) * 18, y: h * 0.35 },
        { x: w * 0.78 + Math.cos(t * 1.5) * 16, y: h * 0.62 },
        { x: w * 0.35, y: h * 0.78 + Math.sin(t * 2) * 12 }
    ];
    taggers.forEach(({ x, y }) => {
    ...
    const orbX = w * 0.72 + Math.cos(t * 3) * 10;
    const orbY = h * 0.28 + Math.sin(t * 3) * 8;
...
/** Neon Kill: dark arena, neon grid, enemy silhouette, pulsing crosshair. */
function drawNeonKillPreview(ctx, w, h, t) {
    ...
    const ex = w * 0.5 + Math.sin(t * 0.9) * 12;
    const ey = h * 0.52;
    ctx.fillStyle = '#ff007f';
    ctx.fillRect(ex - 8, ey - 14, 16, 22);
    ctx.fillStyle = '#fff';
    ctx.fillRect(ex - 3, ey - 10, 6, 4);

    const pulse = 0.55 + Math.sin(t * 5) * 0.25;
...
/** Space Runner: scrolling stars, ship weaving, passing asteroids. */
function drawSpaceRunnerPreview(ctx, w, h, t) {
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    grad.addColorStop(0, '#050818');
    grad.addColorStop(1, '#121a3a');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, w, h);

    for (let i = 0; i < 24; i++) {
        const sx = (i * 47 + t * 40) % w;
        const sy = (i * 29) % h;
        ctx.fillStyle = i % 3 === 0 ? '#fff' : 'rgba(255,255,255,0.5)';
        ctx.fillRect(sx, sy, i % 5 === 0 ? 2 : 1, 1);
    }

    const shipX = w * 0.38 + Math.sin(t * 2.2) * (w * 0.22);
    const shipY = h * 0.55 + Math.sin(t * 3.5) * 8;
    ctx.fillStyle = '#74d2ff';
    ctx.beginPath();
    ctx.moveTo(shipX + 14, shipY);
    ctx.lineTo(shipX - 6, shipY - 7);
    ctx.lineTo(shipX - 2, shipY);
    ctx.lineTo(shipX - 6, shipY + 7);
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = '#ff9bd2';
    ctx.fillRect(shipX - 10, shipY - 2, 6, 4);

    for (let i = 0; i < 3; i++) {
        const ax = w + 20 - ((t * 90 + i * 70) % (w + 40));
        const ay = h * (0.25 + i * 0.22) + Math.sin(t + i) * 6;
        ctx.fillStyle = '#8a7a6a';
        ctx.beginPath();
        ctx.arc(ax, ay, 7 + i * 2, 0, Math.PI * 2);
        ctx.fill();
    }
}

/** Water Royale: pool, bushes, rival soak skirmish. */
function drawWaterRoyalePreview(ctx, w, h, t) {
    ctx.fillStyle = '#7ec850';
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = '#5a9e38';
    ctx.fillRect(0, h * 0.55, w, h * 0.45);
    ctx.fillStyle = '#1bb8ff';
    ctx.fillRect(w * 0.15, h * 0.5, w * 0.7, h * 0.38);
    ctx.fillStyle = 'rgba(255,255,255,0.25)';
    ctx.fillRect(w * 0.2, h * 0.54, w * 0.18, 4);

    const bushes = [0.12, 0.82];
    bushes.forEach(bx => {
        ctx.fillStyle = '#2d6b2a';
        ctx.beginPath();
        ctx.arc(bx * w, h * 0.48, 12, 0, Math.PI * 2);
        ctx.fill();
    });

    const p1x = w * 0.35 + Math.sin(t * 1.6) * 8;
    const p2x = w * 0.65 + Math.cos(t * 1.4) * 8;
    ctx.fillStyle = '#ffd166';
    ctx.fillRect(p1x - 4, h * 0.62 - 8, 8, 14);
    ctx.fillStyle = '#ff6b9d';
    ctx.fillRect(p2x - 4, h * 0.62 - 8, 8, 14);

    ctx.strokeStyle = 'rgba(27, 184, 255, 0.85)';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(p1x + 4, h * 0.58);
    ctx.quadraticCurveTo(p1x + 20, h * 0.52 + Math.sin(t * 8) * 4, p2x - 4, h * 0.58);
    ctx.stroke();
}

/** Fast Eagle: sky, scrolling pipes, flapping bird. */
function drawFastEaglePreview(ctx, w, h, t) {
    const grad = ctx.createLinearGradient(0, 0, 0, h);
    ...
    const scroll = (t * 55) % (w + 50);
    for (let i = 0; i < 2; i++) {
        const px = w + 30 - scroll - i * 90;
        ctx.fillStyle = '#5cb85c';
        ctx.fillRect(px, 0, 22, h * 0.38);
        ctx.fillRect(px, h * 0.58, 22, h * 0.42);
        ctx.fillStyle = '#449944';
        ctx.fillRect(px - 3, h * 0.36, 28, 6);
        ctx.fillRect(px - 3, h * 0.56, 28, 6);
    }

    const birdY = h * 0.46 + Math.sin(t * 6) * 5;
    const wing = Math.sin(t * 12) * 6;
    ctx.fillStyle = '#ffe66d';
    ctx.beginPath();
    ctx.ellipse(w * 0.32, birdY, 10, 7, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#ffb347';
    ctx.beginPath();
    ctx.moveTo(w * 0.32 + 8, birdY);
    ctx.lineTo(w * 0.32 + 16, birdY + wing);
    ctx.lineTo(w * 0.32 + 8, birdY + 4);
    ctx.closePath();
    ctx.fill();
}
...
function tickGamePreview(timestamp) {
    ...
    if (!gamePreviewStart) gamePreviewStart = timestamp;
    const t = (timestamp - gamePreviewStart) / 1000;
    const drawer = GAME_PREVIEW_DRAWERS[gamePreviewActive.key];
    if (drawer) {
        drawer(gamePreviewCtx, gamePreviewCanvas.width, gamePreviewCanvas.height, t);
    }
    gamePreviewFrameId = requestAnimationFrame(tickGamePreview);
}

/** Position the float above the cursor, clamped so it stays on screen. */
function moveGamePreview(clientX, clientY) {
    if (!gamePreviewFloat) return;
    const pad = 12;
    const rect = gamePreviewFloat.getBoundingClientRect();
    const halfW = rect.width / 2 || 116;
    const floatH = rect.height || 160;
    const x = Math.min(window.innerWidth - pad - halfW, Math.max(pad + halfW, clientX));
    const y = Math.max(pad + floatH, Math.min(window.innerHeight - pad, clientY));
    gamePreviewFloat.style.left = `${x}px`;
    gamePreviewFloat.style.top = `${y}px`;
}

/** Show themed preview for the hovered tile and start its animation loop. */
function showGamePreview(tile, clientX, clientY) {
    const key = getGamePreviewKey(tile);
    if (!key || !gamePreviewFloat || !gamePreviewCanvas) return;
    ...
    moveGamePreview(clientX, clientY);
    if (gamePreviewFrameId) cancelAnimationFrame(gamePreviewFrameId);
    gamePreviewFrameId = requestAnimationFrame(tickGamePreview);
}

/** Hide preview and stop the animation loop. */
function hideGamePreview() {
    gamePreviewActive = null;
    gamePreviewStart = 0;
    if (gamePreviewFrameId) {
        cancelAnimationFrame(gamePreviewFrameId);
...
/** Wire mouseenter/move/leave on each `.game-tile` in the games grid. */
function setupGameTilePreviews() {
    if (!gamesPage || !gamePreviewFloat) return;

    gamesPage.querySelectorAll('.game-tile').forEach(tile => {
        tile.addEventListener('mouseenter', (e) => {
            if (gamesPage.classList.contains('hidden')) return;
            showGamePreview(tile, e.clientX, e.clientY);
        });
        tile.addEventListener('mousemove', (e) => {
            if (!gamePreviewActive) return;
